...
* **論理演算子**: `AND`, `OR`, `NOT` (大文字小文字を区別しない)
* **比較演算子**: `=`, `!=`, `<>`, `>`, `>=`, `<`, `<=`
* **メンバーシップ演算子**: `IN` (値リスト), `CONTAINS` (部分文字列 / 配列要素)
...
1. 比較 (`=`, `!=`, `IN`, `CONTAINS` など) と `NOT` が最初に評価される
2. `AND` 条件が次に評価される
3. `OR` 条件が最後に評価される
...
# IN と CONTAINS によるメンバーシップ
status IN ("active", "pending")
tags CONTAINS "golang"        # 文字列では部分一致、配列では要素一致
...
field = /pattern/i  # 大文字小文字を区別しないマッチ（i フラグ）
```
...
- 閉じスラッシュの後に `i` を付けると大文字小文字を区別しない（`/john/i`）
